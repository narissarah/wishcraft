...
/**
 * Gift message encryption (just use standard encrypt/decrypt)
 */
export const encryptGiftMessage = encrypt;
export const decryptGiftMessage = decrypt;

/**
 * Gift message validation and sanitization
 */
export function validateGiftMessage(message: string): boolean {
...
export function logGiftMessageOperation(operation: string, data: any): void {
  console.log(`Gift message ${operation}:`, data);
}

/**
 * Searchable email hash
 */
export const createSearchableEmailHash = createSearchHash;

/**
 * PII encryption aliases for different types
 */
export const encryptPurchasePII = encryptPII;
export const decryptPurchasePII = decryptPII;
export const encryptAddressPII = encryptPII;
export const decryptAddressPII = decryptPII;
export const encryptCollaboratorPII = (data: { email: string; name?: string }) => ({
  email: encryptPII(data.email),
  name: data.name ? encryptPII(data.name) : undefined
});
export const decryptCollaboratorPII = (data: { email: string; name?: string }) => ({
  email: decryptPII(data.email),
  name: data.name ? decryptPII(data.name) : undefined
});